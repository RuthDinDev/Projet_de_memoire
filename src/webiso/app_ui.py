...
if "site1" in st.session_state and "site2" in st.session_state:
    ...
    for idx_site, site in enumerate((site1, site2)):
        with st.expander(f"📄 {site['domaine']}"):
            onglet_meta, onglet_struct, onglet_contenu = st.tabs(["Métadonnées", "Structure", "Contenu"])
            onglet_meta.dataframe(pd.DataFrame(site["meta_list"]), width="stretch", key=f"df_meta_{idx_site}")
            onglet_struct.dataframe(pd.DataFrame(site["struct_list"]), width="stretch", key=f"df_struct_{idx_site}")
            onglet_contenu.dataframe(pd.DataFrame(site["contenu_list"]), width="stretch", key=f"df_contenu_{idx_site}")

    st.header("Graphes formels (NetworkX)")
    for palette_key, graph_key, titre in COUCHES:
        st.subheader(titre)
        col1, col2 = st.columns(2)
        for idx_site, (col, site) in enumerate(zip((col1, col2), (site1, site2))):
